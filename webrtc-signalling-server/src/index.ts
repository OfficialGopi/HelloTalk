...
const callRooms: Map<string, CallRoom> = new Map();
const socketToUser: Map<string, string> = new Map();

// Helper functions
function createCallId(user1: string, user2: string): string {
...
function isUserAvailable(userId: string): boolean {
  ...
  socket.on(
    ...
    (data: { to: string; callType: "audio" | "video" }) => {
      try {
        ...
        console.log(
          `Call initiated from ${caller.id} to ${to}, type: ${callType}`
        );
      } catch (error) {
